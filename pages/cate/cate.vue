<template>
	<view class="container">
		<view class="uni-form-item uni-column">
			<input class="uni-input" focus placeholder="请输入搜索内容" />
		</view>
		<view class="nav-body">
			...
			>
				<view v-for="(foods, index) in classifyData" :key="index" class="box">
					...
					<view :id="i == 0 ? 'first' : ''" class="nav-right-item" v-for="(item, i) in foods.foods" :key="i" @click="cart(item.cat)">
...
<script>
	import classifyData from '../../common/cateTpey.js';
	export default {
		data() {
			return {
				name: 'wkiwi',
...
<style>
	...
	.uni-form-item {background-color: #eee;text-align: center;padding: 8px 0;}
	.uni-form-item .uni-input{background-color: white;width: 650rpx;margin: 0 auto;padding: 6px 5px;border-radius: 3px;font-size: 13px;}
	.nav-body{
		display: flex;
		background: #fff;
		overflow: hidden;
